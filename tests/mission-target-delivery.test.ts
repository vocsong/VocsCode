...
/** A fresh store and Mission from the source's current (clean) HEAD, with one accepted candidate. */
async function startMission(): Promise<void> {
  workspaces = new MissionWorkspaces({ root: path.join(root, `workspaces-${randomUUID()}`), quiescence: { acquire: async () => ({ assertQuiescent: async () => undefined, release: () => undefined }) } });
...
/** The approved target as published now, when the source baseline is delivered without a target refresh. */
const publishedTarget = () => ({ commitSha: git(remote, ['rev-parse', 'refs/heads/target']) }) as TargetObservation;
/** A real server-side hook: the bare remote declines pushes until the returned release runs. */
async function declinePushes(): Promise<() => Promise<void>> {
  const marker = path.join(root, 'decline-pushes');
  await fs.writeFile(marker, '');
  const hook = path.join(remote, 'hooks', 'pre-receive');
  await fs.writeFile(hook, `#!/bin/sh\nif [ -f '${marker.replaceAll('\\', '/')}' ]; then echo 'fixture policy declined this push' >&2; exit 1; fi\nexit 0\n`);
  await fs.chmod(hook, 0o755);
  return () => fs.rm(marker, { force: true });
}
const receiptOf = async (operationId: string) => JSON.parse(await fs.readFile(path.join(root, 'delivery', 'm01', `${operationId}.json`), 'utf8'));
...
beforeEach(async () => {
  ...
  // The baseline is already on the approved target: a PR must contain only Mission work.
...
describe('real remote target refresh through the delivery process boundary', () => {
  it.each(['open_pr', 'merge_pr'] as const)('delivers %s after the target advances past the published baseline, twice, without changing the source', async (endpoint) => {
    ...
    // The published change is exactly the Mission's work on top of the approved target.
    expect(git(remote, ['diff-tree', '-r', '--name-only', newHead, result.commitSha!])).toBe('mission.txt');
  ...
  it('does not parent an unintegrated result to an advanced target, even when its objects were fetched', async () => {
  ...
  it.each([
    { endpoint: 'open_pr', integrated: false },
    { endpoint: 'merge_pr', integrated: true },
  ] as const)('refuses $endpoint delivery that would publish unpushed baseline commits (target integrated: $integrated)', async ({ endpoint, integrated }) => {
    // The user's clean checkout carries a feature commit the approved target never received.
    await write(source, 'feature.txt', 'unpushed feature work\n'); git(source, ['add', '.']); git(source, ['commit', '-m', 'Unpushed feature work']);
    await startMission();
    let observation = publishedTarget();
    if (integrated) { await advance('diverged remote\n'); observation = await observe('diverged'); await integrate(observation); }
    else integration = await workspaces.materializeAccepted(integration.id, integration.fingerprint);
    const fixture = await delivery(observation, endpoint); const before = await preserved();
    await expect(fixture.service.deliver(fixture.request)).rejects.toThrow(/baseline has 1 local commit\(s\) that origin\/target does not contain.*narrow delivery to a local commit/);
    expect(fixture.calls.some((args) => ['commit-tree', 'push'].includes(args[1]) || args[0] === 'gh')).toBe(false);
    expect(git(remote, ['for-each-ref', '--format=%(refname)', 'refs/heads'])).toBe('refs/heads/target');
    expect(git(remote, ['rev-parse', 'refs/heads/target'])).toBe(observation.commitSha);
    expect(await preserved()).toEqual(before);
    // A local commit remains deliverable: it publishes nothing.
    fixture.request.mission.deliveryPolicy = localMissionDeliveryPolicy(); fixture.request.operationId = 'deliver-local';
    const local = await fixture.service.deliver(fixture.request);
    expect(local).toMatchObject({ status: 'delivered', endpoint: 'local_commit' });
    expect(git(source, ['show', '-s', '--format=%P', local.commitSha!])).toBe(baseline.revision.baseCommitSha);
    expect(fixture.calls.some((args) => args[1] === 'push' || args[0] === 'gh')).toBe(false);
  }, 120_000);

  it('scopes policy holds to every path the PR against the integrated target actually changes', async () => {
    // Upstream edits a baseline file and the Mission reverts it: baseline→accepted cannot see
    // that path, but the published PR (target→accepted) changes it.
    await write(upstream, 'local.txt', 'upstream edit\n'); git(upstream, ['add', '.']); git(upstream, ['commit', '-m', 'Upstream edit']);
    git(upstream, ['push', 'origin', 'HEAD:refs/heads/target']);
    const observation = await observe('upstream-edit'); await integrate(observation);
    const reverter = await workspaces.provision({ missionId: 'm01', baseline, role: 'worker', attemptId: 'a2' });
    expect(await fs.readFile(path.join(reverter.cwd, 'local.txt'), 'utf8')).toBe('upstream edit\n');
    await write(reverter.cwd, 'local.txt', 'local baseline\n');
    const candidate = await workspaces.captureCandidate(reverter.id, 'a2');
    expect((await workspaces.integrate({ missionId: 'm01', candidateId: candidate.id, expectedAccepted: await workspaces.acceptedRevision('m01'), check: async () => true })).status).toBe('accepted');
    integration = await workspaces.materializeAccepted(integration.id, integration.fingerprint);
    const scope = await workspaces.acceptedChangedPaths('m01');
    expect(scope).toEqual(['local.txt', 'mission.txt']);
    const fixture = await delivery(observation, 'open_pr');
    const result = await fixture.service.deliver(fixture.request);
    expect(result.status).toBe('delivered');
    expect(git(remote, ['diff-tree', '-r', '--name-only', observation.commitSha, result.commitSha!]).split('\n')).toEqual(scope);
  }, 120_000);

  it('records a declined push as a definite failure the same operation retries, never forcing', async () => {
    const release = await declinePushes();
    integration = await workspaces.materializeAccepted(integration.id, integration.fingerprint);
    const fixture = await delivery(publishedTarget(), 'open_pr');
    for (let attempt = 0; attempt < 2; attempt++) {
      await expect(fixture.service.deliver(fixture.request)).rejects.toThrow(/did not publish mission\/m01-delivery/);
      expect(await receiptOf('deliver-one')).toMatchObject({ stage: 'committed', commitSha: expect.any(String) });
      expect((await receiptOf('deliver-one')).pendingAction).toBeUndefined();
      expect(git(remote, ['for-each-ref', '--format=%(refname)', 'refs/heads'])).toBe('refs/heads/target');
      expect(fixture.prs.size).toBe(0);
    }
    await release();
    const result = await fixture.service.deliver(fixture.request);
    expect(result).toMatchObject({ status: 'delivered', pullRequestUrl: expect.any(String) });
    expect(git(remote, ['rev-parse', 'refs/heads/mission/m01-delivery'])).toBe(result.commitSha);
    expect(fixture.calls.filter((args) => args[1] === 'push')).toHaveLength(3);
    expect(fixture.calls.filter((args) => args[1] === 'commit-tree')).toHaveLength(1);
    expect(fixture.prs.size).toBe(1);
    expect(fixture.calls.some((args) => args.some((arg) => /^(?:--force|-f|--force-with-lease)$/.test(arg)))).toBe(false);
  }, 120_000);

  it('lets a new delivery operation reuse the identical commit a failed operation left behind', async () => {
    const release = await declinePushes();
    integration = await workspaces.materializeAccepted(integration.id, integration.fingerprint);
    const fixture = await delivery(publishedTarget(), 'merge_pr');
    await expect(fixture.service.deliver(fixture.request)).rejects.toThrow(/did not publish/);
    const branch = missionDeliveryBranch(fixture.request.mission);
    const commit = git(source, ['rev-parse', `refs/heads/${branch}`]);
    await release();
    fixture.request.operationId = 'deliver-two';
    const result = await fixture.service.deliver(fixture.request);
    expect(result).toMatchObject({ status: 'delivered', commitSha: commit, mergedCommitSha: commit });
    expect(await receiptOf('deliver-two')).toMatchObject({ commitSha: commit, commitOperationId: 'deliver-one', stage: 'delivered' });
    expect(git(remote, ['rev-parse', 'refs/heads/target'])).toBe(commit);
    expect((await fixture.service.deliver(fixture.request)).commitSha).toBe(commit);
    expect(fixture.calls.filter((args) => args[1] === 'commit-tree')).toHaveLength(1);
    expect(fixture.calls.filter((args) => args[2] === 'create')).toHaveLength(1);
    expect(fixture.calls.filter((args) => args[2] === 'merge')).toHaveLength(1);
  }, 120_000);

  it('treats a push whose acknowledgment was lost as done once the remote holds the exact commit', async () => {
    integration = await workspaces.materializeAccepted(integration.id, integration.fingerprint);
    const fixture = await delivery(publishedTarget(), 'open_pr');
    const transport = vi.mocked(runtime.runCapture).getMockImplementation()!;
    let lost = false;
    vi.mocked(runtime.runCapture).mockImplementation(async (file, args, options) => {
      const result = await transport(file, args, options);
      if (!lost && args.includes('push')) { lost = true; expect(result.code).toBe(0); throw new Error('lost push acknowledgment'); }
      return result;
    });
    const result = await fixture.service.deliver(fixture.request);
