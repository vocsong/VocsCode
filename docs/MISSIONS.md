...
Present npm `typecheck`, `test` and `build` scripts are baseline gates when no manifest is supplied. An unrecognized test reporter requires explicit configuration.
