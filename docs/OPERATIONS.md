...
Renderer failures reach the same file: React render errors, uncaught exceptions and unhandled rejections arrive as `renderer error at <source>: <message>` followed by the stack (via `app:rendererError`, capped at 20 per minute), and the renderer's own `console.error`/`console.warn` output as `[renderer console] …` — in every build, not just development. A crashed renderer is reloaded a bounded number of times (`renderer process gone: … reloading (n/limit)`). Chromium helper processes dying (`renderer process gone`, `GPU process gone`), an unresponsive window, a failed page load and a broken preload script are logged too.
