...
| **Pi** | `pi --mode rpc` + bundled approvals and MCP-bridge extensions | interactive | pi's registry: Anthropic, OpenAI, Codex OAuth, Google, DeepSeek, OpenRouter, OpenCode Go, Ollama, custom | injected — the bridge extension registers each MCP tool with pi |
