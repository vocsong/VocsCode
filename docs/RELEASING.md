...
2. Open the ship PR: `develop` → `master`. **Use "Create a merge commit"** — do not squash this one, or the bot only sees a single opaque merge message. Merge it.
3. release-please opens a `chore(release): vX.Y.Z` PR against `master` with the version bump and changelog. Review the notes, edit the PR description to add lines if you want, merge it.
...
5. Sync back: open PR `master` → `develop` and merge it (merge commit), so `develop` gets the version bump and changelog.
