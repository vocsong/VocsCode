...
export type NativeMessage =
  ...
  | { role: 'assistant'; text: string; reasoning?: string; toolCalls: { id: string; name: string; args: Record<string, unknown> }[] }
...
function toAnthropicMessages(history: NativeMessage[]): Anthropic.MessageParam[] {
...
export async function anthropicStep(p: StepParams): Promise<StepResult> {
  ...
  const body: Anthropic.MessageCreateParamsStreaming = {
    ...
    messages: toAnthropicMessages(p.history),
    tools,
    stream: true
  };
  if (adaptive) {
...
export async function openaiStep(p: StepParams): Promise<StepResult> {
  ...
  const reasoningCapable = p.provider.kind === 'openai' || /^(o\d|gpt-5)/.test(p.model);
  ...
  for await (const chunk of stream) {
    ...
    if (usage) {
      result.usage.inputTokens = usage.prompt_tokens ?? 0;
      result.usage.outputTokens = usage.completion_tokens ?? 0;
      result.usage.cacheReadTokens = usage.prompt_tokens_details?.cached_tokens ?? usage.prompt_cache_hit_tokens ?? 0;
