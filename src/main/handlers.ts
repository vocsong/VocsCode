...
import { globalStoreInfo, inspectServer, mergeById, normalizeStdio, projectInfo, readProjectMcp, readStore, resolveVars, secretKeyFor, toMcpJsonTable, writeProjectMcp, cuaStatus, type GitnexusIndexer, type GitnexusIndexReason } from './mcp';
...
export function createHandlerRegistry(deps: HandlerDeps): HandlerRegistry {
  ...
  // Computer use. The renderer only ever reads status; the mode and opt-in live in settings.
