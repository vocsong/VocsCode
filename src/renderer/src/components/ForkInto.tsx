...
export function ForkIntoDropdown({ session, onForked, trigger }: {
  ...
  const fork = (harness: HarnessId) => {
    void invoke('sessions:fork', { id: session.id, harness }).then((f) => {
      if (f) {
      ...
      {(close) => (
        <>
          ...
          {[h, ...HARNESSES.filter((x) => x.id !== session.config.harness)].map((x) => {
            ...
            return (
              <MenuItem key={x.id} active={x.id === session.config.harness} disabled={unavailable} onClick={() => { close(); fork(x.id); }}>
