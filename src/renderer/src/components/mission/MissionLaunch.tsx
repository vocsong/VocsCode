...
import { HARNESS_BY_ID, PERMISSION_MODE_LABELS } from '../../../../shared/harness-meta';
import { validatePresetEligibility } from '../../../../shared/mission-config';
...
export function MissionLaunch({ choices }: { choices: React.ReactNode }) {
  ...
  return <Modal title="New session" onClose={close} width={620} footer={<><Button variant="ghost" onClick={close}>Cancel</Button><Button variant="primary" disabled={busy || !configured || !objective.trim() || !root || !modes.includes(permissionMode)} onClick={() => void launch()}>{busy && <Spinner />} Start Mission</Button></>}>
    ...
    <div className="mission-ui mission-launch">
      ...
      <Field label="Principal engineer (T5 only)"><select aria-label="Principal engineer (T5 only)" value={override} onChange={(e) => setOverride(e.target.value)}><option value="">{config?.defaultLeadPresetId ? `Project default · ${config.presets.find((p) => p.id === config.defaultLeadPresetId)?.name ?? 'unavailable'}` : 'No default principal engineer configured'}</option>{leads.map((p) => <option key={p.id} value={p.id}>{p.name} · {p.model.provider}/{p.model.model}</option>)}</select></Field>
      ...
      {(!configured || configError) && <div className="callout warn" role="alert">{configError ?? 'Mission is unconfigured. Choose an enabled T5 principal engineer in Settings → Mission. No lower-tier fallback is used.'} <Button size="sm" onClick={() => { close(); useStore.getState().setView('settings'); }}>Configure Mission</Button></div>}
